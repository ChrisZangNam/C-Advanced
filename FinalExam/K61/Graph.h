...
void addVertex2(Graph graph, int id, JRB tree)
{
    JRB node = jrb_find_int(graph.vertices, id);

    if (node == NULL)
    {
        jrb_insert_int(graph.vertices, id, new_jval_v(tree));
    }
...
int shortedPath(Graph graph, int s, int t, int *path, int *length)//length: so dinh ma no di qua = so luong path
{
