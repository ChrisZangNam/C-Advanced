...
int all_Path(Graph graph, int start, int stop, int path[][100], int a[]);
...
void addEdge(Graph graph, int v1, int v2, int weight)
{
    ...
    {
        int cur_weight = getEdgeValue(graph, v1, v2);
        int new_weight;
        ...
        jrb_insert_int(tree, v2, new_jval_i(new_weight));
        ...
        jrb_delete_node(node1);
        jrb_insert_int(tree, v1, new_jval_i(new_weight));
    }
}
...
int shortestPath(Graph graph, int s, int t, int *path, int *length) //length: so dinh ma no di qua = so luong path
{
