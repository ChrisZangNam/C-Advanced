...
// Soundex Algorithms "https://www.rosettacode.org/wiki/Soundex#C"
...
// Cac ham su dung thuat toan soundex
...
void stringProcess(char *input, char *output1)
{
	int i;
...
gboolean checkSoundex(Widgets *wg, GdkEvent  *event, gpointer   user_data)
{
	...
	if(key.keyval == GDK_KEY_Tab && isCheckSoundex == 1)
	{
		...
		if(strlen(input) > 0) 
		{
			stringProcess(input, s1);
			if(strcmp(entryget, s1) != 0) 
			{
				gtk_entry_set_text(wg, s1);
			}
			showMessage("Press TAB to see next words!");
		}	
		return 1;
	}
	return 0;
...
void refreshBtn_click(GtkWidget *button, Widgets *app) 
{
	refreshData();
